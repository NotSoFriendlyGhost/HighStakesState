...
void redLeftAuton(){
  ...
  // chassis.pid_wait();
  // chassis.pid_odom_set({{},rev, DRIVE_SPEED});
  // chassis.pid_wait();
  // intake.move(127);
  ...
  chassis.pid_turn_set({-7.191,49.441},fwd,TURN_SPEED);
  chassis.pid_wait();
  chassis.pid_odom_set({{-7.191,49.441},fwd,DRIVE_SPEED});
  chassis.pid_wait();
...
void leftRed(){
  ...
  chassis.pid_wait();
  chassis.pid_odom_set({{-28.252,25.755}, rev, 30});
  chassis.pid_wait();
  mogo.set_value(1);
  // pros::delay(250);
  ...
  // intake.move(127);
  // pros::Task task{[=]{
  //   double GV = opticalSensor.get_hue();
  //   if(GV<240&&GV>200){
  //     if(pros::)
  ...
  // }};
  chassis.pid_turn_set({-8.278,39.626}, fwd, TURN_SPEED);
  chassis.pid_wait();
  chassis.pid_odom_set({{-8.278,39.626}, fwd, 63});
  chassis.pid_wait();

  chassis.pid_turn_set({-23.536,47.116}, fwd, TURN_SPEED);
  chassis.pid_wait();
  chassis.pid_odom_set({{-23.536,47.116}, fwd, 63});
  chassis.pid_wait();

  chassis.pid_turn_set({-24.091,5.781}, fwd, TURN_SPEED);
  chassis.pid_wait();
  chassis.pid_odom_set({{-24.091,5.781}, fwd, 63});
  chassis.pid_wait();
}
